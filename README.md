...
每个数字就是一个**可调的旋钮**。训练就是拧这 570 万个旋钮。
...
```
训练前：570 万个随机数字 → ppl 1200 → 输出 "荪忆在南阳时始雪何回流估苋……"（乱码）

训练后：570 万个调好的数字 → ppl 15  → 输出 "明月明镜，余雪花不歇。夜寒歌钟，舞袖落云间。"
```
...
```
{
  "token_emb.weight":   [0.0023, -0.0142,  0.0087, ...]  一共 570 万个 float32
...
御定全唐诗（900 卷 JSON，繁体）
  ...
  字符级分词器（3357 个不重复汉字 → ID）
        │
        ▼  get_batch()
  随机采样：输入前 256 字，预测下一个字
        │
        ▼  MiniGPT
  6 层 Transformer（570 万参数，含权重绑定）
        │
        ▼  AdamW + 交叉熵损失
...
从全集中**随机抽取** 32 段（`batch_size=32`），每段 256 个字（`block_size=256`）。
...
输入 ID（B, T）
     ...
     ├──→ Position Embedding（256 个位置 → 256 维向量）
     │
     ▼
...
self.pos_emb = nn.Embedding(block_size, d_model)     # 256 个位置 → 256 维
```
...
> 💡 **为什么 100 轮？**
> 数据量小（10 万字），模型容量也小（570 万参数），100 轮不会过拟合。配合**余弦学习率**（前期大步快学、后期小步精调）和 **warmup**（前 10% 步数 lr 从 0 线性爬升，避免初期方向不稳），收敛更深更稳。
...
def generate(self, idx, max_new_tokens, temperature=1.0):
    for _ in range(max_new_tokens):
        idx_cond = idx[:, -self.block_size:]      # 只保留最后 256 字
...
```
vocab: 3357 | params: 5.7M | device: cpu | block: 256
steps_per_epoch: 12
